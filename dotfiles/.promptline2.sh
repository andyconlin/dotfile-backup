...
function __promptline_host {
  ...
  if [ $only_if_ssh -eq 0 -o -n "${SSH_CLIENT}" ]; then
    ...
    if [[ -n ${ZSH_VERSION-} ]]; then print %m; elif [[ -n ${FISH_VERSION-} ]]; then hostname -s; else printf "%s" "$(scutil --get ComputerName)"; fi
  fi
}
...
function __promptline {
  ...
  local b_bg="${wrap}48;5;13${end_wrap}"
  local b_sep_fg="${wrap}38;5;13${end_wrap}"
